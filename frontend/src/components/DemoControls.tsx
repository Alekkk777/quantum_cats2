...
export function DemoControls({ phase, usedFallback, onNext, onBack, onReset }: DemoControlsProps) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-ink/90 text-paper shadow-[0_12px_30px_oklch(0.20_0.01_270/0.25)]">
      ...
      {usedFallback && (
        <span className="font-mono text-[9.5px] tracking-[0.08em] uppercase text-paper-3 border border-ink-3 rounded-full px-2 py-px">
          demo fallback
        </span>
      )}
      ...
      <button onClick={onBack} disabled={idx <= 0} className="px-2.5 py-1 rounded-full border border-ink-3 bg-ink-2 text-paper text-[12px] disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Previous step">Back</button>
      ...
      <button onClick={onReset} className="px-2.5 py-1 rounded-full border border-ink-3 bg-ink-2 text-paper text-[12px]" aria-label="Restart demo">Reset</button>
    </div>
  );
}
