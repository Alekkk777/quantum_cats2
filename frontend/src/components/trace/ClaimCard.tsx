...
export function ClaimCard({ claim, onChallenge }: { claim: Claim; onChallenge?: (claim: Claim) => void }) {
  return (
    <div
      ...
      <div>
        <div className="flex flex-wrap items-baseline gap-2 mb-1">
          ...
          <span className={`font-mono text-[10px] tracking-[0.1em] uppercase font-semibold ${verdictFg[claim.verdict]}`}>
            {verdictLabel[claim.verdict]} - {claim.label}
          </span>
        </div>
        ...
        {onChallenge && (
          <button
            className="mt-2 font-mono text-[10.5px] uppercase tracking-[0.08em] text-ink-3 hover:text-ink"
            onClick={() => onChallenge(claim)}
          >
